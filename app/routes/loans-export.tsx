import { exportLoans, type ExportFormat } from "~/api/loans";
...
import type { Route } from "./+types/loans-export";

/**
 * `GET /loans/export?format=csv|xlsx&…` — the loan book as a spreadsheet,
...
const STATUSES = ["pending", "active", "arrears", "repaid", "rejected"];
...
export async function loader({ request }: Route.LoaderArgs) {
  ...
  try {
    ...
    return await asDownload(upstream, {
      kind: format,
      filename: `loans-${accraDay()}.${format}`,
