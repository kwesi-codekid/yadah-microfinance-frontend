import { exportCollections, type ExportFormat } from "~/api/reports";
import { asDownload, downloadFailure } from "~/lib/download.server";
import { accraDay } from "~/lib/format";
import { requireOffice, withAuth } from "~/lib/session.server";
import type { Route } from "./+types/report-collections-export";

/**
 * `GET /reports/collections?format=csv|xlsx` as a download, proxied with the
...
export async function loader({ request }: Route.LoaderArgs) {
  ...
  try {
    const { data: upstream, headers } = await withAuth(request, (token) =>
      exportCollections(token, { from: day("from"), to: day("to") }, format),
    );

    return await asDownload(upstream, {
      kind: format,
      filename: `collections-${accraDay()}.${format}`,
